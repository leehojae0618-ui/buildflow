...
필요한 Delivery Surface로만 포함할 수 있다.
...
- BuildFlow는 각 기능의 Capability를 AUTO, PARTIAL, CONSENT_REQUIRED, MANUAL, EXPERT, UNSUPPORTED로 설명한다.
- 공식 Constraint 단계는 AUTO, PARTIAL, CONSENT_REQUIRED, MANUAL, EXPERT, UNSUPPORTED이며 Consent와 Manual을 구분한다.
...
Marketplace의 제품 공유 단위는 BPS와 호환되는 `AI Agent Package`다.
Package는 다음을 포함한다.
...
Package는 Secret과 사용자 개인정보를 포함하지 않는 안전한 Version
Snapshot이어야 하며, 다른 사용자는 지원되는 범위에서 AI Agent를
검토·재구축·설치할 수 있다.
