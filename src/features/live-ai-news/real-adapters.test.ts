...
import {
  ...
  toGroqRecipeProcessorPort,
  toNewsRecipeInputPort,
...
describe("Recipe Execution Contract adapters (roadmap Step 2)", () => {
  it("toNewsRecipeInputPort returns the same items as calling the source directly", async () => {
    const source = new OpenAiNewsRssSource({
      fetchImpl: async () => ({ ok: true, status: 200, text: async () => rss }),
    });
    const direct = await source.fetchNews();
    const viaPort = await toNewsRecipeInputPort(source)();

    expect(viaPort).toEqual(direct);
  });

  it("toGroqRecipeProcessorPort returns the same summary as calling the summarizer directly", async () => {
    const items = selectRecentImportantNews(parseOpenAiNewsRss(rss), 2);
    const summarizer = new GroqSummaryAdapter({
      client: { chat: { completions: { create: async () => ({ choices: [{ message: { content: "1. New GPT model release\n- summary line" } }] }) } } },
    });

    const direct = await summarizer.summarize({ items });
    const viaPort = await toGroqRecipeProcessorPort(summarizer)({ items });

    expect(viaPort).toEqual(direct);
  });

  it("toSlackDigestRecipeDestinationPort writes through the same guarded service call as runApprovedSlackDigestWrite", async () => {
    const adapter = new FakePipedreamConnectAdapter({ slackAccounts: [{ safeExternalReference: "pd_verified" }] });
    const destination = toSlackDigestRecipeDestinationPort({
      environment: guardedEnvironment,
      identityProvider: guardedIdentity,
      adapter,
      idempotency: new InMemorySlackWriteIdempotency(),
    });
    ...
    const result = await destination({
      ...
      recipeId: "recipe.ai-news-slack-digest",
      targetConfigurationReference: "approved-channel",
      requestId: "slack-digest-contract-adapter-12345678",
      payload: "[BuildFlow AI 뉴스 테스트]\n\nContract adapter equivalence check\n\nSource: OpenAI News\nGenerated by BuildFlow",
    });

    expect(result).toMatchObject({ ok: true, connectionState: "CONNECTED_VERIFIED", evidence: { actionType: "AI_NEWS_DIGEST", status: "SUCCEEDED" } });
  });

  it("toSlackDigestRecipeDestinationPort rejects a request for any other recipeId before reaching the guarded service", async () => {
    const destination = toSlackDigestRecipeDestinationPort();

    await expect(
      destination({
        approved: true,
        recipeId: "recipe.some-other-recipe",
        targetConfigurationReference: "approved-channel",
        requestId: "slack-digest-contract-adapter-87654321",
        payload: "irrelevant",
      }),
    ).rejects.toThrow("UNSUPPORTED_RECIPE_FOR_SLACK_DIGEST_DESTINATION");
  });
});
