...
import { getAutonomousBuildSession, resumeAutonomousBuildSession, startAutonomousBuildSession } from "../actions";
...
export function AutonomousSessionSummary({ projectId, initialSession }: { projectId: string; initialSession?: { id: string; status: string } | null }) {
  const [session, setSession] = useState<{ id: string; status: string } | null>(initialSession ?? null); const [message, setMessage] = useState("");
  ...
  async function resume(event: "CREDENTIALS_READY" | "CONSENT_GRANTED" | "APPROVAL_GRANTED" | "PROVISIONED" | "VERIFIED") { if (!session) return; const result = await resumeAutonomousBuildSession(projectId, session.id, event); if (!result.ok) setMessage(result.error); else setSession({ ...session, status: result.status }); }
  const copy = session ? statusCopy[session.status] ?? { title: "AI가 서비스를 준비하고 있습니다", detail: "자동으로 다음 단계를 진행합니다.", progress: 45 } : null;
  return <section className="mt-5 border border-cyan-400/20 bg-cyan-400/5 p-5"><p className="text-xs uppercase tracking-[0.2em] text-cyan-300">진행 상황</p>{!session ? <><h3 className="mt-3 text-lg font-medium">준비가 되면 AI가 구축을 시작합니다</h3><p className="mt-2 text-sm text-zinc-400">페이지를 닫아도 저장된 세션에서 이어갈 수 있습니다.</p><button type="button" onClick={start} className="mt-4 border border-cyan-300/40 px-3 py-2 text-sm text-cyan-200">구축 시작</button></> : <><h3 className="mt-3 text-lg font-medium text-white">{copy?.title}</h3><p className="mt-2 text-sm text-zinc-300">{copy?.detail}</p><div className="mt-5 h-2 overflow-hidden bg-zinc-800"><div className="h-full bg-cyan-300 transition-all" style={{ width: `${copy?.progress ?? 0}%` }} /></div><p className="mt-2 text-right text-xs text-zinc-400">약 {copy?.progress ?? 0}% 진행</p><div className="mt-5 flex flex-wrap gap-2">{timeline.map((item, index) => <span key={item} className={`px-2 py-1 text-xs ${index < Math.ceil((copy?.progress ?? 0) / 20) ? "text-emerald-200" : "text-zinc-500"}`}>{index < Math.ceil((copy?.progress ?? 0) / 20) ? "✓" : "○"} {item}</span>)}</div>{copy?.waiting && <button type="button" onClick={() => resume(session.status === "WAITING_FOR_CREDENTIAL" ? "CREDENTIALS_READY" : session.status === "WAITING_FOR_CONSENT" ? "CONSENT_GRANTED" : "APPROVAL_GRANTED")} className="mt-4 border border-amber-300/40 px-3 py-2 text-sm text-amber-200">완료 후 자동으로 이어가기</button>}<details className="mt-5 text-xs text-zinc-500"><summary className="cursor-pointer">기술 상세 보기</summary><p className="mt-2">Session 상태: {session.status}</p></details></>}{message && <p className="mt-3 text-xs text-red-300" role="alert">{message}</p>}</section>;
}
