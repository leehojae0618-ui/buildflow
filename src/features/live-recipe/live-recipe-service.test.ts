...
import { InMemorySlackWriteIdempotency, markSlackConnectReturnedUnverified, prepareSlackConnectLink, prepareSlackTestAction, runApprovedSlackDigestWrite, runApprovedSlackTestWrite, type LiveRecipeServiceDependencies } from "./live-recipe-service";
import { slackConnectionTestMessage } from "./types";

const recipeId = "recipe.ai-news-slack-digest" as const;
...
describe("FIRST-LIVE-RECIPE-E2E-001 Phase A", () => {
  ...
  it("runs one approved AI news digest Slack write through the digest-only action boundary", async () => {
    const adapter = new FakePipedreamConnectAdapter({ slackAccounts: [verifiedSlackAccount] });
    ...
    }, dependencies({ environment: environment({ liveConnectEnabled: true, liveSlackWriteEnabled: true }), adapter }));

    expect(result).toMatchObject({
      ok: true,
      connectionState: "CONNECTED_VERIFIED",
      value: { slackTimestamp: "1786779999.000001" },
      evidence: { actionType: "AI_NEWS_DIGEST", status: "SUCCEEDED" },
    });
    expect(adapter.invocationCount.runSlackTestAction).toBe(1);
  });

  it("blocks AI news digest Slack write when the write switch is disabled", async () => {
    const adapter = new FakePipedreamConnectAdapter({ slackAccounts: [verifiedSlackAccount] });
    const result = await runApprovedSlackDigestWrite({
      approved: true,
      recipeId,
      targetConfigurationReference: "approved-channel",
      requestId: "slack-digest-request-disabled",
      message: "[BuildFlow AI 뉴스 테스트]\n\nSource: OpenAI News\nGenerated by BuildFlow",
    }, dependencies({ environment: environment({ liveConnectEnabled: true, liveSlackWriteEnabled: false }), adapter }));

    expect(result).toMatchObject({ ok: false, errorCode: "WRITE_DISABLED", evidence: { actionType: "AI_NEWS_DIGEST", status: "BLOCKED" } });
    expect(adapter.invocationCount.runSlackTestAction).toBe(0);
  });

  it("rejects missing approval, a wrong destination, missing destination configuration, and a missing digest adapter before a write", async () => {
    const adapter = new FakePipedreamConnectAdapter({ slackAccounts: [verifiedSlackAccount] });
    const base = { recipeId, targetConfigurationReference: "approved-channel", requestId: "slack-digest-request-rejected", message: "[BuildFlow AI 뉴스 테스트] Source: OpenAI News" };
    const enabled = dependencies({ environment: environment({ liveConnectEnabled: true, liveSlackWriteEnabled: true }), adapter });

    await expect(runApprovedSlackDigestWrite(base as never, enabled)).resolves.toMatchObject({ ok: false, errorCode: "WRITE_NOT_APPROVED" });
    await expect(runApprovedSlackDigestWrite({ ...base, approved: false } as never, enabled)).resolves.toMatchObject({ ok: false, errorCode: "WRITE_NOT_APPROVED" });
    await expect(runApprovedSlackDigestWrite({ ...base, approved: true, targetConfigurationReference: "other-channel" }, enabled)).resolves.toMatchObject({ ok: false, errorCode: "DESTINATION_NOT_APPROVED" });
    await expect(runApprovedSlackDigestWrite({ ...base, approved: true }, dependencies({ environment: environment({ liveConnectEnabled: true, liveSlackWriteEnabled: true, approvedSlackChannelId: undefined }), adapter }))).resolves.toMatchObject({ ok: false, errorCode: "CONFIGURATION_MISSING" });
    const missingAdapter = new FakePipedreamConnectAdapter({ slackAccounts: [verifiedSlackAccount] });
    missingAdapter.runSlackDigestAction = undefined;
    await expect(runApprovedSlackDigestWrite({ ...base, approved: true, requestId: "slack-digest-request-no-adapter" }, dependencies({ environment: environment({ liveConnectEnabled: true, liveSlackWriteEnabled: true }), adapter: missingAdapter }))).resolves.toMatchObject({ ok: false, errorCode: "EXTERNAL_ACTION_FAILED" });
    expect(adapter.invocationCount.runSlackTestAction).toBe(0);
    expect(missingAdapter.invocationCount.runSlackTestAction).toBe(0);
  });

  it("fails closed for an unverified account, an identity failure, adapter rejection, and duplicate digest requests", async () => {
    const request = { approved: true as const, recipeId, targetConfigurationReference: "approved-channel", requestId: "slack-digest-request-duplicate", message: "[BuildFlow AI 뉴스 테스트] Source: OpenAI News" };
    const noAccount = new FakePipedreamConnectAdapter();
    await expect(runApprovedSlackDigestWrite(request, dependencies({ environment: environment({ liveConnectEnabled: true, liveSlackWriteEnabled: true }), adapter: noAccount }))).resolves.toMatchObject({ ok: false, errorCode: "SLACK_ACCOUNT_NOT_VERIFIED" });
    const rejectedIdentity: LiveUserIdentityProvider = { resolve: async () => ({ ok: false, errorCode: "AUTH_REQUIRED" }) };
    await expect(runApprovedSlackDigestWrite({ ...request, requestId: "slack-digest-request-identity" }, dependencies({ environment: environment({ liveConnectEnabled: true, liveSlackWriteEnabled: true }), identityProvider: rejectedIdentity, adapter: new FakePipedreamConnectAdapter({ slackAccounts: [verifiedSlackAccount] }) }))).resolves.toMatchObject({ ok: false, errorCode: "AUTH_REQUIRED" });
    const rejectingAdapter = new FakePipedreamConnectAdapter({ slackAccounts: [verifiedSlackAccount] });
    rejectingAdapter.runSlackDigestAction = async () => { throw new Error("rejected"); };
    await expect(runApprovedSlackDigestWrite({ ...request, requestId: "slack-digest-request-rejected-adapter" }, dependencies({ environment: environment({ liveConnectEnabled: true, liveSlackWriteEnabled: true }), adapter: rejectingAdapter }))).resolves.toMatchObject({ ok: false, errorCode: "EXTERNAL_ACTION_FAILED" });
    const wrongAccountAdapter = new FakePipedreamConnectAdapter({ slackAccounts: [verifiedSlackAccount] });
    wrongAccountAdapter.runSlackDigestAction = async () => { throw new Error("SLACK_ACCOUNT_NOT_VERIFIED"); };
    await expect(runApprovedSlackDigestWrite({ ...request, requestId: "slack-digest-request-wrong-account" }, dependencies({ environment: environment({ liveConnectEnabled: true, liveSlackWriteEnabled: true }), adapter: wrongAccountAdapter }))).resolves.toMatchObject({ ok: false, errorCode: "SLACK_ACCOUNT_NOT_VERIFIED" });
    const duplicateAdapter = new FakePipedreamConnectAdapter({ slackAccounts: [verifiedSlackAccount] });
    const duplicateDependencies = dependencies({ environment: environment({ liveConnectEnabled: true, liveSlackWriteEnabled: true }), adapter: duplicateAdapter, idempotency: new InMemorySlackWriteIdempotency() });
    await expect(runApprovedSlackDigestWrite(request, duplicateDependencies)).resolves.toMatchObject({ ok: true });
    await expect(runApprovedSlackDigestWrite(request, duplicateDependencies)).resolves.toMatchObject({ ok: false, errorCode: "DUPLICATE_REQUEST" });
    expect(duplicateAdapter.invocationCount.runSlackTestAction).toBe(1);
  });
