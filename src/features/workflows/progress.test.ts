...
import { calculateProgressFromSteps, calculateWorkflowProgress, difficultyLabel, workflowStatusForProgress, workflowStatusLabel } from "./progress";
...
describe("workflow progress", () => {
  ...
  it.each([0, 25, 50, 75, 100])("supports %s percent", (percent) => { const total = 4; const steps = Array.from({ length: total }, (_, index) => ({ is_completed: index < percent / 25 })); expect(calculateProgressFromSteps(steps).progressPercent).toBe(percent); });
