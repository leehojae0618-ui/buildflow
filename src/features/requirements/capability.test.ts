...
describe("constraint capability foundation", () => {
  it("calculates automatic and user-action capabilities", () => {
    ...
    expect(snapshot.capabilities.some((capability) => capability.level === "CONSENT_REQUIRED")).toBe(true);
    expect(snapshot.capabilitySummary.automation).toBeLessThan(100);
  });
  it("keeps calculator extensible for new component capabilities", () => {
    ...
    const summary = calculateCapabilitySummary([...capabilities, { id: "future", label: "Future", level: "EXPERT", reason: "전문가 검토", requiresConsent: false }]);
    expect(summary.expert).toBe(1);
