...
export function LoginForm({ next }: { next?: string }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 text-xs text-zinc-600"><span className="h-px flex-1 bg-zinc-800" />또는<span className="h-px flex-1 bg-zinc-800" /></div>
      <form action={signInWithGoogle}><input type="hidden" name="next" value={next ?? "/app"} /><button type="submit" className="w-full border border-zinc-700 px-4 py-3 text-sm font-medium text-zinc-200 hover:border-cyan-400/50 hover:text-cyan-200">Google로 계속하기</button></form>
