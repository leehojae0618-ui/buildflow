...
import { prepareAiNewsDigestRun, requestApprovedAiNewsDigestRun, type AiNewsDigestGateErrorCode, type AiNewsDigestPreview, type AiNewsDigestRunResult } from "@/features/live-ai-news/actions";
...
export function RecipeFirstExperience() {
  ...
  function resetDigestRun() { setDigestPreview(null); setDigestRunResult(null); setDigestRunning(false); }
  ...
  async function approveDigestRun() {
    ...
    const result = await requestApprovedAiNewsDigestRun();
    setDigestRunResult(result);
  ...
  return <main className="min-h-screen bg-[#081112] text-zinc-100">
    ...
    <section className="mx-auto w-full max-w-6xl px-5 py-10 sm:px-8 sm:py-14"><div className="flex flex-col justify-between gap-5 border-b border-zinc-800 pb-6 sm:flex-row sm:items-end"><div><p className="text-xs font-medium tracking-[0.16em] text-emerald-200">RECOMMENDATIONS</p><h2 className="mt-2 text-2xl font-semibold text-white">{submittedGoal ? view.heading : "목표를 입력하면 추천을 시작합니다"}</h2>{submittedGoal && <p className="mt-2 text-sm text-zinc-400">{view.goalSummary}</p>}</div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setCostPreference((value) => value === "FREE_FIRST" ? "BALANCED" : "FREE_FIRST")} className="border border-zinc-700 px-3 py-2 text-xs text-zinc-300 hover:border-emerald-200/50">{costPreference === "FREE_FIRST" ? "무료/저비용 우선" : "균형 추천"}</button><button type="button" onClick={() => setConnectionPreference((value) => value === "FEWER_CONNECTIONS" ? "FULL_CAPABILITY" : "FEWER_CONNECTIONS")} className="border border-zinc-700 px-3 py-2 text-xs text-zinc-300 hover:border-emerald-200/50">{connectionPreference === "FEWER_CONNECTIONS" ? "적은 연결 우선" : "기능 범위 우선"}</button></div></div>
      ...
      {buildPackage && <BuildPreparation buildPackage={buildPackage} onEngineSelect={setSelectedEngine} onStartConnection={requestSlackConnection} connectionNotice={connectionNotice} onPrepareTestWrite={prepareTestWrite} onApproveTestWrite={approveTestWrite} testWriteNotice={testWriteNotice} awaitingTestWriteApproval={awaitingTestWriteApproval} onAnswerClarification={answerClarification} digestPreview={digestPreview} digestRunResult={digestRunResult} digestRunning={digestRunning} onPrepareDigestRun={prepareDigestRun} onApproveDigestRun={approveDigestRun} onClose={() => { setSelected(null); setSelectedEngine(undefined); setConnectionNotice(null); setTestWriteNotice(null); setAwaitingTestWriteApproval(false); setClarificationAnswers({}); resetDigestRun(); }} />}
    </section>
  </main>;
}
...
function BuildPreparation({ buildPackage, onEngineSelect, onStartConnection, connectionNotice, onPrepareTestWrite, onApproveTestWrite, testWriteNotice, awaitingTestWriteApproval, onAnswerClarification, digestPreview, digestRunResult, digestRunning, onPrepareDigestRun, onApproveDigestRun, onClose }: { buildPackage: ReturnType<typeof createBuildPackage>; onEngineSelect: (engine: ExecutionEngine) => void; onStartConnection: () => void; connectionNotice: string | null; onPrepareTestWrite: () => void; onApproveTestWrite: () => void; testWriteNotice: string | null; awaitingTestWriteApproval: boolean; onAnswerClarification: (id: string, value: string) => void; digestPreview: AiNewsDigestPreview | null; digestRunResult: AiNewsDigestRunResult | null; digestRunning: boolean; onPrepareDigestRun: () => void; onApproveDigestRun: () => void; onClose: () => void }) {
