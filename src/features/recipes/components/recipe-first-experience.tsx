...
import { createBuildPackage } from "../build-package";
...
export function RecipeFirstExperience() {
  ...
  const buildPackage = useMemo(() => selected ? createBuildPackage({ recipe: selected.recipe, intent: view.recommendation.intent, selectedEngine }) : null, [selected, selectedEngine, view.recommendation.intent]);

  function submit() { setSelected(null); setSelectedEngine(undefined); setConnectionNotice(null); setTestWriteNotice(null); setAwaitingTestWriteApproval(false); setSubmittedGoal(goal.trim()); }
  function selectRecipe(item: RankedRecipe) { setSelected(item); setSelectedEngine(undefined); setConnectionNotice(null); setTestWriteNotice(null); setAwaitingTestWriteApproval(false); }
...
function BuildPreparation({ buildPackage, onEngineSelect, onStartConnection, connectionNotice, onPrepareTestWrite, onApproveTestWrite, testWriteNotice, awaitingTestWriteApproval, onClose }: { buildPackage: ReturnType<typeof createBuildPackage>; onEngineSelect: (engine: ExecutionEngine) => void; onStartConnection: () => void; connectionNotice: string | null; onPrepareTestWrite: () => void; onApproveTestWrite: () => void; testWriteNotice: string | null; awaitingTestWriteApproval: boolean; onClose: () => void }) {
