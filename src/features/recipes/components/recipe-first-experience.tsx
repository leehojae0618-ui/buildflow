...
export function RecipeFirstExperience() {
  ...
  const [digestSteps, setDigestSteps] = useState<Array<{ label: string; detail: string }>>([]);
  ...
  function resetDigestRun() { setDigestPreview(null); setDigestSteps([]); setDigestRunResult(null); setDigestRunning(false); }
  ...
  async function approveDigestRun() {
    ...
    setDigestSteps((steps) => [...steps, { label: "1. 뉴스 수집", detail: `${fetchResult.selectedItemCount}건 확인` }]);
    ...
    setDigestSteps((steps) => [...steps, { label: "2. AI 요약", detail: `${summaryResult.summaryLineCount}줄 생성` }]);
    ...
    setDigestSteps((steps) => [...steps, { label: "3. Slack 전송", detail: writeResult.safeSlackReference }]);
...
function BuildPreparation({ buildPackage, onEngineSelect, onStartConnection, connectionNotice, onPrepareTestWrite, onApproveTestWrite, testWriteNotice, awaitingTestWriteApproval, onAnswerClarification, digestPreview, digestSteps, digestRunResult, digestRunning, onPrepareDigestRun, onApproveDigestRun, onClose }: { buildPackage: ReturnType<typeof createBuildPackage>; onEngineSelect: (engine: ExecutionEngine) => void; onStartConnection: () => void; connectionNotice: string | null; onPrepareTestWrite: () => void; onApproveTestWrite: () => void; testWriteNotice: string | null; awaitingTestWriteApproval: boolean; onAnswerClarification: (id: string, value: string) => void; digestPreview: AiNewsDigestPreview | null; digestSteps: Array<{ label: string; detail: string }>; digestRunResult: AiNewsDigestRunResult | null; digestRunning: boolean; onPrepareDigestRun: () => void; onApproveDigestRun: () => void; onClose: () => void }) {
  return <section aria-live="polite" className="border-t border-emerald-100/20 py-8"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-medium tracking-[0.16em] text-emerald-200">BUILD PREPARATION</p><h2 className="mt-2 text-2xl font-semibold text-white">구축 준비</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-400">선택한 Recipe를 실제 연결 전까지 구성한 결과입니다. Package 상태: <span className="text-emerald-100">{buildPackage.status}</span></p></div><button type="button" onClick={onClose} className="border border-zinc-700 px-3 py-2 text-sm text-zinc-300 hover:border-zinc-400">닫기</button></div>
    ...
    {buildPackage.recipeId === "recipe.ai-news-slack-digest" && <section className="mt-8 border-t border-zinc-800 pt-6"><h3 className="text-sm font-semibold text-white">Recipe 실행</h3><p className="mt-2 text-sm leading-6 text-zinc-400">승인 전에는 어떤 외부 호출도 일어나지 않습니다. 아래에서 실행 대상을 먼저 확인하세요.</p>
      ...
      {(digestSteps.length > 0 || digestRunning) && <ol aria-live="polite" className="mt-4 grid gap-2">{digestSteps.map((step) => <li key={step.label} className="flex items-center gap-3 border border-emerald-200/20 bg-emerald-200/5 px-4 py-3 text-sm text-emerald-100"><span>✓</span><span>{step.label} — {step.detail}</span></li>)}{digestRunning && digestSteps.length < 3 && <li className="flex items-center gap-3 border border-zinc-800 bg-[#0b1514] px-4 py-3 text-sm text-zinc-300"><span>○</span><span>{["1. 뉴스 수집", "2. AI 요약", "3. Slack 전송"][digestSteps.length]} 진행 중...</span></li>}</ol>}
      {digestRunResult && (digestRunResult.ok
        ? <p role="status" className="mt-4 border border-emerald-200/30 bg-emerald-200/5 p-4 text-sm text-emerald-100">실행 완료: 뉴스 {digestRunResult.selectedItemCount}건, 요약 {digestRunResult.summaryLineCount}줄, Slack 참조 {digestRunResult.safeSlackReference}</p>
        : <p role="status" className="mt-4 border border-amber-200/30 bg-amber-200/5 p-4 text-sm text-amber-100">{digestGateErrorLabel[digestRunResult.errorCode]}</p>)}
    </section>}
